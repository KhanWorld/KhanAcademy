...
function KnowledgeMap(params) {
    ...
    this.init = function(params) {
		...
		var exerciseTitles = null;
		$.ajax({
			type: "GET",
			url: "/khan-exercises/exercises/lang.js",
			async:false,
			success: function(data){
				table = eval(data);
				exerciseTitles = table["titles"]["nl"];
				$(function(){
					$(".exercise-title>a").each(function(){
						if($(this).text() in exerciseTitles){
							$(this).text(exerciseTitles[$(this).text()]);
						}
					})
				})
			}
        ...
        $.each(graph_dict_data, function(idx, exercise) {
            ...
            // Update map graph
			var tempNode = exerciseModel.toJSON();
			if(exerciseTitles != null){
				if(tempNode.display_name in exerciseTitles){
					tempNode.display_name = exerciseTitles[tempNode.display_name];
				}
        	}
